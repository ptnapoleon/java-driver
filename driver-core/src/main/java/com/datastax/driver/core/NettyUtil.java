...
class NettyUtil {
    ...
    static {
        ...
        if (!SHADED) {
            try {
                Class<?> epool = Class.forName("io.netty.channel.epoll.Epoll");
                if (FORCE_NIO) {
                ...
                } else if (!(Boolean)epool.getMethod("isAvailable").invoke(null)) {
                    LOGGER.warn("Found Netty's native epoll transport in the classpath, but epoll is not available. "
                        + "Using NIO instead.", (Throwable)epool.getMethod("unavailabilityCause").invoke(null));
                } else {
